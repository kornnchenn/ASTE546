...
double gather(double li, const dvector &f) {
	//idk what this notation is the int i = (int) li, does it cast li as an int? why do that?
	//wait is it finding the remainder? like if li was 11.1 it would give di = 0.1 ?
	int i = (int) li;
	double di = li-i;
...
int main()
{
   ...
   double vth_i = 500;
    ...
    for (int p=0;p<ions.np_alloc;p++) {
		...
		//define a pointer called part of the type Particle? 
	...
	//out << "something," << ions.getCurrent(world) << ", something else" << -eles.getCurrent(world) << endl;
	...
	for (world.ts=0;world.ts<20000;world.ts++) {
		...
		if (world.ts%100==0)
			...
			out <<world.ts << "," << sim_time << "," << ions.np << "," << ions.getCurrent(world) << "," << eles.np << "," << -eles.getCurrent(world) << "\n";
